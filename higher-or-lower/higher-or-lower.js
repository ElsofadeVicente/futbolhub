...
async function loadModeData(mode) {
  if (mode.multiFile) {
    ...
    for (const file of mode.files) {
      try {
        const res = await fetch(`${HOL_CONFIG.dataPath}${mode.folder}${file}`);
    ...
    if (loaded === 0) { console.error('❌ [HOL] Ningún archivo cargado.'); return {}; }
    return allPlayers;
  } else {
    try {
      const res = await fetch(`${HOL_CONFIG.dataPath}${mode.file}`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
